...
async function boot() {
	...
	await type("Bienvenu(e) dans le terminal Enigma", {
		initialWait: 3000
	});

	await type(["> SET TERMINAL/BOOT", "Chargement........................"], {
		lineWait: 1000
	});
	await type(
		[
	...
	await type(["> SET TERMINAL/LOGON", "AUTHENTIFICATION DE L'UTILISATEUR CHECK"], {
		lineWait: 1000,
		finalWait: 3000
	});
...
async function login() {
	clear();
	let user = await prompt("Nom d'utilisateur:");
	let password = await prompt("Mot de passe:", true);

	if (user === USER && password === PW) {
		await pause();
		say("AUTHENTIFICATION REUSSIE");
		await alert("AUTHENTIFICATION REUSSIE");
		clear();
		return main();
	} else {
		await type([
